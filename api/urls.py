...
if settings.DEBUG:
    for version, patterns in version_map_dict.items():
        title = 'FEP API Adapter'
        base_url = '/api'
        sv = get_schema_view(
            openapi.Info(
                title=title,
                default_version=version,
                description=f"FEP {version} Api List",
                terms_of_service="https://www.google.com/policies/terms/",
                contact=openapi.Contact(email="ghl92479@gmail.com"),
                license=openapi.License(name="Cafe Liense"),
            ),
            permission_classes=(permissions.AllowAny,),
        )
        urlpatterns = [re_path(r'^' + version + '/swagger/(?P<format>\.json|\.yaml)$', sv.without_ui(cache_timeout=0)),
                       re_path(r'^' + version + '/swagger/$', sv.with_ui('swagger', cache_timeout=0)),
                       re_path(r'^' + version + '/redoc/$', sv.with_ui('redoc', cache_timeout=0))] + urlpatterns
